...
#define DRIVER_SPI_BUS_SPEED    25000000  /* Default SPI bus speed */
#endif
...
#define CMD_READ_CHIPID         (0x9FU)
#define CMD_READ_FLAG_STATUS    (0x05U)
...
static ARM_FLASH_INFO FlashInfo = {
  NULL,
  W25_FLASH_SECTOR_COUNT,
  W25_FLASH_SECTOR_SIZE,
  W25_FLASH_PAGE_SIZE,
  W25_FLASH_PROGRAM_UNIT,
  W25_FLASH_ERASED_VALUE,
...
static int32_t ReadData (uint32_t addr, void *data, uint32_t cnt) {
  ...
  if ((addr > (W25_FLASH_SECTOR_COUNT * W25_FLASH_SECTOR_SIZE)) || (data == NULL)) {
    ...
    if (status == ARM_DRIVER_OK) {
      ...
      if (status == ARM_DRIVER_OK) {
        ...
        if (status == ARM_DRIVER_OK) {
          ...
          n = W25_FLASH_PAGE_SIZE - (addr % W25_FLASH_PAGE_SIZE);
...
static ARM_FLASH_INFO * GetInfo (void) {
	...
	uint8_t buf[4] = {0};
	uint32_t chipId = 0;
	static struct _ARM_FLASH_INFO flashInfo = {0};
  ...
  if (status == ARM_DRIVER_OK) {
    /* Set command */
    buf[0] = CMD_READ_CHIPID;

    /* Send command and receive register value */
		status = ptrSPI->Send(&buf[0], 1U);
		status = ptrSPI->Receive(&buf[1], 3U);

    if (status == ARM_DRIVER_OK) {
      /* Wait till transfer done */
      while (ptrSPI->GetDataCount() != 3U);
    }
  }	
	/* Deselect Slave */
  ptrSPI->Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_INACTIVE);
	...
	{
		case 0x20: 	/* w25q512 */
			  flashInfo.sector_count = 1024 << 4;
				break;
		case 0x19:  /* w25q256 */
				flashInfo.sector_count = 512 << 4;
				break;
		case 0x18:  /* w25q128 */
				flashInfo.sector_count = 256 << 4;
				break;
		case 0x17: 	/* w25q64 */
			  flashInfo.sector_count = 128 << 4;
				break;
		case 0x16:  /* w25q32 */
				flashInfo.sector_count = 64 << 4;
				break;
		case 0x15:  /* w25q16 */
				flashInfo.sector_count = 32 << 4;
				break;
		case 0x14:  /* w25q80 */
				flashInfo.sector_count = 16 << 4;
				break;
		case 0x13:  /* w25q40 */
				flashInfo.sector_count = 8 << 4;
				break;
		case 0x12:  /* w25q20 */
				flashInfo.sector_count = 4 << 4;
				break;
		case 0x11:  /* w25q10 */
				flashInfo.sector_count = 2 << 4;
				break;
	...
	flashInfo.sector_info = FlashInfo.sector_info;
	flashInfo.page_size = FlashInfo.page_size;
	flashInfo.sector_size = FlashInfo.sector_size;
	flashInfo.erased_value = FlashInfo.erased_value;
	flashInfo.program_unit = FlashInfo.program_unit;
	flashInfo.reserved[0] = buf[1];
	flashInfo.reserved[1] = buf[2];
	flashInfo.reserved[2] = buf[3];
