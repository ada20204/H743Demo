 ...
 *------------------------------------------------------------------------------
 * Name:    USBD_User_CDC_ACM_RNDIS_VETH_0.c
 ...
 *
 * USBD_User_CDC_ACM_RNDIS_VETH_0.c implements the application specific
...
// Global functions exported by this module
       ARM_ETH_LINK_STATE RNDIS0_GetLinkState  (void);
       int32_t            RNDIS0_SendFrame     (const uint8_t *frame, uint32_t len);
       int32_t            RNDIS0_ReadFrame     (      uint8_t *frame, uint32_t len);
       uint32_t           RNDIS0_GetRxFrameSize(void);
...
/**
  \fn          ARM_ETH_LINK_STATE RNDIS0_GetLinkState (void)
...
/**
  \fn          int32_t RNDIS0_SendFrame (const uint8_t *frame, uint32_t len, uint32_t flags)
...
/**
  \fn          int32_t RNDIS0_ReadFrame (uint8_t *frame, uint32_t len)
...
/**
  \fn          uint32_t RNDIS0_GetRxFrameSize (void)
...
uint32_t RNDIS0_GetRxFrameSize (void) {
  ...
  avail_data_len = (uint32_t)USBD_CDC_ACM_DataAvailable (0);
