...
static int32_t ReadData (uint32_t addr, void *data, uint32_t cnt) {
  FlashStatus.busy  = 1U;
  uint32_t result = SFUD_SUCCESS;
  result = sfud_read(nor_flash, addr, cnt, (uint8_t* )data);
  FlashStatus.busy  = 0U;
  return result == SFUD_SUCCESS ? cnt : ARM_DRIVER_ERROR;
}
...
static int32_t ProgramData (uint32_t addr, const void *data, uint32_t cnt) {
  FlashStatus.busy  = 1U;
  uint32_t result = SFUD_SUCCESS;
  sfud_erase_write(nor_flash, addr, cnt, data);
  FlashStatus.busy  = 0U;
  return result == SFUD_SUCCESS ? cnt : ARM_DRIVER_ERROR;
}
...
static int32_t EraseSector (uint32_t addr) {
  FlashStatus.busy  = 1U;
  uint32_t result = SFUD_SUCCESS;
  result = sfud_erase(nor_flash, addr, W25_FLASH_SECTOR_SIZE);
  FlashStatus.busy  = 0U;
  return result == SFUD_SUCCESS ? ARM_DRIVER_OK : ARM_DRIVER_ERROR;
}
...
static int32_t EraseChip (void) {
  FlashStatus.busy  = 1U;
  uint32_t result = SFUD_SUCCESS;
  result = sfud_chip_erase(nor_flash);
  FlashStatus.busy  = 0U;
  return result == SFUD_SUCCESS ? ARM_DRIVER_OK : ARM_DRIVER_ERROR_UNSUPPORTED;
}
